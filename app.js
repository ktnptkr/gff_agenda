...
function initTabs() {
    const tabs = { all: document.getElementById('tab-all'), saved: document.getElementById('tab-saved'), live: document.getElementById('tab-live') };
    const actionsBar = document.getElementById('agenda-actions');

    const resetTabs = () => {
        Object.values(tabs).forEach(t => {
            t.className = t.id === 'tab-live' ? "px-4 py-1.5 rounded-md text-sm font-semibold text-white hover:text-red-300 transition whitespace-nowrap flex items-center gap-1.5" : "px-4 py-1.5 rounded-md text-sm font-semibold text-white hover:text-slate-200 transition whitespace-nowrap";
        });
        if(actionsBar) actionsBar.classList.add('hidden');
    };

    tabs.all.addEventListener('click', () => { viewMode = "all"; resetTabs(); tabs.all.className = "px-4 py-1.5 rounded-md text-sm font-semibold bg-white text-navy shadow transition whitespace-nowrap"; updateDropdowns(); renderAgenda(); });
    tabs.saved.addEventListener('click', () => { viewMode = "saved"; resetTabs(); tabs.saved.className = "px-4 py-1.5 rounded-md text-sm font-semibold bg-white text-navy shadow transition whitespace-nowrap"; if(actionsBar) actionsBar.classList.remove('hidden'); updateDropdowns(); renderAgenda(); });
    tabs.live.addEventListener('click', () => { viewMode = "live"; resetTabs(); tabs.live.className = "px-4 py-1.5 rounded-md text-sm font-semibold bg-white text-red-600 shadow transition whitespace-nowrap flex items-center gap-1.5"; updateDropdowns(); renderAgenda(); });
    ...
    if(notifBtn) {
        notifBtn.addEventListener('click', () => {
            ...
            Notification.requestPermission().then(permission => {
                if (permission === "granted") { showToast("Reminders active!", "🔔"); notifBtn.innerText = "🔔 Reminders Active"; notifBtn.disabled = true; }
            });
        });
    }
}
...
function initDateButtons() {
    ...
    html += dates.map(date => {
        ...
        return `<button onclick="setDate('${date}')" class="date-btn px-4 py-1.5 rounded-full text-sm font-semibold transition flex-shrink-0 ${date === currentDate ? 'bg-navy text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200'}" data-date="${date}">${d.getDate().toString().padStart(2, '0')} ${d.toLocaleString('default', { month: 'short' })}</button>`;
    }).join('');
...
function updateDropdowns() {
    ...
    const extractUnique = (data, key, splitRegex) => {
        ...
        data.forEach(e => {
            if (e[key]) {
                if (splitRegex) e[key].split(splitRegex).forEach(v => { let clean = v.replace(/\(.*?\)/g, '').trim(); if (clean) set.add(clean); });
                else set.add(e[key].trim());
            }
        });
        return [...set].sort();
    };
...
function populateSelect(id, options, currentValue, defaultLabel) {
    ...
    el.value = options.includes(currentValue) ? currentValue : 'all';
}
...
function parseTimes(dateStr, timeStr) {
    ...
    return { start: new Date(`${dateStr}T${parts[0].trim()}:00+05:30`), end: new Date(`${dateStr}T${(parts[1] || parts[0]).trim()}:00+05:30`) };
}
...
function checkLiveStatus(dateStr, timeStr) {
    ...
    const isImminent = start.getTime() > now && (start.getTime() - now) <= (30 * 60 * 1000); 
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm transition-all ${status.isPast ? 'past-event' : 'hover:shadow-md'} ${status.isLive ? 'ring-2 ring-red-500/50' : ''}">
                <div class="flex justify-between items-start mb-2">
                    <div class="flex gap-2 items-center flex-wrap">
                        ${status.isLive ? '<span class="text-[10px] font-bold px-2 py-0.5 bg-red-100 text-red-600 rounded-md uppercase tracking-wide flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-red-500"></span> LIVE NOW</span>' : ''}
                    ...
                    <div class="flex items-center gap-1 flex-shrink-0">
                        <button onclick="shareSession('${event.id}', event)" class="p-1.5 rounded-full text-slate-400 hover:text-brand hover:bg-slate-100 transition" aria-label="Share">
...
function closeMapModal() {
    const mapModal = document.getElementById('map-modal');
    const mapOverlay = document.getElementById('map-overlay');
    mapOverlay.classList.add('opacity-0'); mapModal.classList.add('translate-y-full');
    setTimeout(() => mapOverlay.classList.add('hidden'), 300);
}

function exportCalendar() {
    const savedEvents = AGENDA_DATA.filter(e => savedSessionIds.includes(e.id));
    if (savedEvents.length === 0) { showToast("No saved sessions.", "⚠️"); return; }
    let ics = "BEGIN:VCALENDAR\nVERSION:2.0\n";
    savedEvents.forEach(e => {
        const { start, end } = parseTimes(e.Date, e.Time);
        ics += "BEGIN:VEVENT\nSUMMARY:" + e["Activity Name"] + "\nDTSTART:" + start.toISOString().replace(/[-:]/g, '').split('.')[0] + "Z\nDTEND:" + end.toISOString().replace(/[-:]/g, '').split('.')[0] + "Z\nEND:VEVENT\n";
    });
    ics += "END:VCALENDAR";
    const link = document.createElement('a');
    link.href = window.URL.createObjectURL(new Blob([ics], { type: 'text/calendar' }));
    link.setAttribute('download', 'GFF_Agenda.ics');
    link.click();
    showToast("Downloaded .ICS file", "📥");
}

function shareItinerary() {
    if (savedSessionIds.length === 0) { showToast("No saved sessions.", "⚠️"); return; }
    const url = `${window.location.origin}${window.location.pathname}#saved=${savedSessionIds.join(',')}`;
    if (navigator.share) navigator.share({ title: 'My GFF Itinerary', url }).catch(()=>{});
...
function shareSession(id, eventObj) {
    ...
    if (navigator.share) navigator.share({ title: event?.["Activity Name"], url }).catch(()=>{});
    else navigator.clipboard.writeText(url).then(() => showToast("Session link copied!", "🔗"));
}
...
function checkUpcomingReminders() {
    ...
    savedSessionIds.forEach(id => {
        ...
        if ((start.getTime() - now) / (1000 * 60) >= 9 && (start.getTime() - now) / (1000 * 60) <= 11) {
            if (!localStorage.getItem(`notified_${id}`)) {
                new Notification(`Starting Soon: ${event["Activity Name"]}`, { body: `At ${event["Location / Room"] || 'Main Stage'}` });
...
function toggleSave(id, eventObj) {
    ...
    if (savedSessionIds.includes(id)) {
        ...
        if (hasConflict) { showToast("Conflict: Overlaps with saved session!", "⚠️"); }
        else { savedSessionIds.push(id); showToast("Added to My Agenda", "✅"); }
    }
    localStorage.setItem('gff_saved_sessions', JSON.stringify(savedSessionIds));
...
function openDrawer(id) {
    ...
    drawerContent.innerHTML = `
        ...
        <div class="flex flex-wrap items-center gap-4 text-sm text-slate-600 mb-6 font-medium bg-slate-50 p-3 rounded-lg border border-slate-100">
            ...
            <span class="text-brand cursor-pointer hover:underline" onclick="openVenueMap('${event["Location / Room"] || 'Main Hall'}')">📍 ${event["Location / Room"] || 'TBA'} (Map)</span>
        </div>
    ...
    setTimeout(() => { drawerOverlay.classList.remove('opacity-0'); drawer.classList.remove('translate-y-full'); }, 10);
}
